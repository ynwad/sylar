...
template<class T, class FromStr = sylar::LexicalCast<std::string, T>, class ToStr = sylar::LexicalCast<T, std::string>>
...
public:
    ...
    ConfigVar(const std::string& name
        ...
        :ConfigVarBase(name, descriptions)
        ,m_val(default_value){
    }
    ...
    std::string toString() override{
        try{
            ...
            SYLAR_LOG_ERROR(SYLAR_LOG_ROOT()) << "ConfigVar::to_string exception"
...
public:
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name,
        ...
        if(name.find_first_not_of("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz._0123456789")
                != std::string::npos){
            SYLAR_LOG_ERROR(SYLAR_LOG_ROOT()) << "Lookup name valid " << name;
